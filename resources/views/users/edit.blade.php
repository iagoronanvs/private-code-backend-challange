            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ isset($user) ? route('users.update',['id' => $user->id]) : route('users.store') }}">
                        ...
                        <div class="form-group row">
                            <label for="permissions" class="col-md-4 col-form-label text-md-right">{{ __('Grupo') }}</label>

                            <div class="col-md-6">

                                <select class="form-control" name="roles[]" multiple>
                                @foreach($roles as $role)
                                    <option @if(isset($userRole) && in_array($role, $userRole)) selected @endif value="{{ $role }}">{{ $role }}</option>
                                @endforeach
                                </select>
